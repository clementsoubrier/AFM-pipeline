...
def plot_centerline_align(center_list, invers_mat,delta_mat,distmat,list0,list1,dicname,diclistname,datadirec,epsilon):
    initdata0=''
    initdata1=''
    for elem0 in list0:
        for elem1 in list1:
            ...
            if data0!=initdata0:
                dic0=np.load(datadirec+data0+dicname, allow_pickle=True)['arr_0'].item()
                mask_list0=np.load(datadirec+data0+diclistname, allow_pickle=True)['arr_0']
                initdata0=data0
            if data1!=initdata1:
                dic1=np.load(datadirec+data1+dicname, allow_pickle=True)['arr_0'].item()
                mask_list1=np.load(datadirec+data1+diclistname, allow_pickle=True)['arr_0']
                initdata1=data1
            plot_2_center(dic0,mask_list0,elem0,dic1,mask_list1,elem1,center_list,invers_mat,delta_mat,distmat,epsilon)
        
        

def plot_2_center(dic0,mask_list0,num0,dic1,mask_list1,num1,center_list,invers_mat,delta_mat,distmat,epsilon):
...
def run_mds_plot(center_list,dist_mat,dicname,listindnam,title,datadirec, mask=None, ROI_list=None):
    ...
    for i in trange(len(pos)):
        ...
        if i==0 or oldname!=infos[1]:
            change_data=True
            oldname=infos[1]
            dic=np.load(datadirec+infos[1]+dicname, allow_pickle=True)['arr_0'].item()
            # ROI_dict=np.load(datadirec+infos[1]+ROI_name, allow_pickle=True)['arr_0'].item()
            mask_list=np.load(datadirec+infos[1]+'masks_list.npz', allow_pickle=True)['arr_0']
            ROI_mask_list=np.load(datadirec+infos[1]+listindnam, allow_pickle=True)['arr_0']
...
def run_mds_plot_2_data_time(center_list,dist_mat,base_data,evoldata,dicname,masklistname,datadirec):
    ...
    for i in range(lenlis):
        if center_list[i,1] in [base_data,evoldata]:
                mask[i]=1
    ...
    dic=np.load(datadirec+evoldata+dicname, allow_pickle=True)['arr_0'].item()
    # ROI_dict=np.load(infos[1]+ROI_name, allow_pickle=True)['arr_0'].item()
    mask_list=np.load(datadirec+evoldata+masklistname, allow_pickle=True)['arr_0']
...
def run_mds_plot_INH_time(center_list,dist_mat,dicname,datadirec):
    ...
    dic=np.load(datadirec+'WT_INH_700min_2014/'+dicname, allow_pickle=True)['arr_0'].item()
    # ROI_dict=np.load(infos[1]+ROI_name, allow_pickle=True)['arr_0'].item()
    mask_list=np.load(datadirec+'WT_INH_700min_2014/'+'masks_list.npz', allow_pickle=True)['arr_0']
...
def plot_growth_rate_INH(dicname,masklistname,Roiname,datadirect):
    ...
    dic=np.load(datadirect+data+dicname, allow_pickle=True)['arr_0'].item()
    mask_list=np.load(datadirect+data+masklistname, allow_pickle=True)['arr_0']
    ROI_dic=np.load(datadirect+data+Roiname, allow_pickle=True)['arr_0'].item()
...
if __name__ == "__main__":
    ...
    A=np.load(dir_cent+'centerline_list_all.npy')
    B=np.load(dir_cent+'distance_matrix_all.npy')
    C=np.load(dir_cent+'inversion_matrix_all.npy')
    D=np.load(dir_cent+'delta_matrix_all.npy')
    ...
    # plot_centerline_align(A,C,D,B,[7],[9],dic_name,mask_list_name,data_direc,epsilon_penal)
    ...
    #         if A[i,1]!=olddata:
    #             ROI_dic=np.load(data_direc+A[i,1]+ROI_name, allow_pickle=True)['arr_0'].item()
    #         if len(ROI_dic[A[i,3]]['Mask IDs'])>7:
    #             mask[i]=1
    # run_mds_plot(A,B,dic_name,indexlistname,'add a title',data_direc,mask=mask)#,ROI_list=['ROI 2','ROI 4','ROI 5','ROI 12','ROI 6','ROI 9','ROI 45']
