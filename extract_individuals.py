...
def get_individuals(adj_mat,current_node,ROI_num=1,root=0):
    """
    Performs a depth first traversal of the binary tree represented by the 
    ...
    if node_list.size==1: 
        ...
        successors, ROI_num = get_individuals(adj_mat,next_node,ROI_num=ROI_num)
...
#%% Obtain ROI dictionary from the adjacency matrix
...
for root in roots:
    individuals = get_individuals(adj_mat,root,ROI_num=starting_num,root=root)[0]
...
#%% Save the output ROI dictionary

dict_filename = 'ROI_dict.npy' 
np.save(dictionary_dir + dict_filename,ROI_dict)
