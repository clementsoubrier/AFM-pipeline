...
def track_simulation():
    ...
    for elem in dic :
        ...
        lin[-1,:] = old_features
        # feat_num = [len(detect_zeros(uv_array[i]))for i in range(len(uv_array))]
        # if not feat_num == sorted(feat_num):
        #     print(feat_num)
        for i,vals in enumerate(uv_array[-2::-1,:,0]):
            ...
            if np.all(dist_arg_list[1:,1]!=dist_arg_list[:-1,1]):
                ...
                # dist_list = old_list[dist_arg_list[:,0]]- new_list(dist_arg_list[:,1])
        print(lin)
